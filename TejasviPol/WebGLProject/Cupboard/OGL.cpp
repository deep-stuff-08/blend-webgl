...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) 
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName,
		TEXT("Tejasvi Ramesh Pol : Animate 2D coloured shapes"),
...
int initialize(void)
{
	...
	if (loadGLTexture(&texture_cupboard, MAKEINTRESOURCE(IDBITMAP_CUPBOARD)) == FALSE)
	{
		fprintf(gpFile, "Failed to load cupboard texture");
		uninitialize();
		return -6;
	}

	if (loadGLTexture(&texture_cupboard_door, MAKEINTRESOURCE(IDBITMAP_CUPBOARD_DOOR)) == FALSE)
	{
		fprintf(gpFile, "Failed to load cupboard door texture");
		uninitialize();
		return -6;
	}
...
void display(void)
{
	...
	glVertex3f(0.8f, 1.8f, -1.0f); // right top

	glTexCoord2f(1.0, 0.0);
	glVertex3f(-0.8f, 1.8f, -1.0f);
	...
	glTexCoord2f(1.0, 0.0);
	glVertex3f(-0.8f, -1.8f, -1.0f); // bottom cha left top
	...
	glTexCoord2f(0.0, 0.0);//4
	glVertex3f(-0.8f, -1.8f, -1.0f);

	glTexCoord2f(1.0, 0.0);//3
	glVertex3f(0.8f, -1.8f, -1.0f);
	...
	glTexCoord2f(1.0, 0.0);
	glVertex3f(-0.8f, 1.8f, -1.0f); // -y sir said
	...
	// texture
	glBindTexture(GL_TEXTURE_2D, texture_cupboard_door);
...
void uninitialize(void)
{
	...
	glDeleteTextures(1, &texture_cupboard);
	if (gpFile)
	{
