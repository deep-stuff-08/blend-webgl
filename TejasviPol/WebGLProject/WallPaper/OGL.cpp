...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow) 
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName,
		TEXT("Tejasvi Ramesh Pol : painting Texture on Quad"),
...
int initialize(void)
{
	...
	if (loadGLTexture(&texture_painting, MAKEINTRESOURCE(IDBITMAP_PAINTING)) == FALSE)
	{
		fprintf(gpFile, "Failed to load painting texture");
		uninitialize();
		return -6;
	}
...
void uninitialize(void)
{
	...
	glDeleteTextures(1, &texture_painting);
	if (gpFile)
	{
